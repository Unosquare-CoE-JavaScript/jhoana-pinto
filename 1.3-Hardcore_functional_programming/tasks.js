...
postTitle(1).fork(console.error, console.log)
...
const commentsForPost = id =>
  getPost(id)
  .chain( post => getComments(post)
        .map( comments => Object.assign({comments}, posts)) 
    )

  
commentsForPost(2).fork(console.error,console.log)
