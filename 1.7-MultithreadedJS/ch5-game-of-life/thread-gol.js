    ...
    iterate(minX, minY, maxX, maxY) {
        ...
        for (let x = minX; x < maxX; x++) {
            for (let y = minY; y < maxY; y++) {
                ...
                const newCell = alive === 3 || (cell && alive === 2) ? 1 :
                    0;
...
if (isMainThread) {
    ...
    const sharedImageBuf8 =
        new Uint8ClampedArray(sharedMemory, imageOffset, 4 * SIZE *
            SIZE);
    ...
    for (let i = 0; i < THREADS; i++) {
        const worker = new Worker('thread-gol.js', {
            name: `golworker-${i}`
        });
        worker.postMessage({
    ...
    function runCoord() {
        ...
        for (let x = 0; x < SIZE; x++) {
            for (let y = 0; y < SIZE; y++) {
                sharedImageBuf[SIZE * x + y] = cells[SIZE * x + y] ? BLACK
                    : WHITE;
            }
        }
        self.postMessage({});
